        ...
        try:
            ...
            while self.socket.fileno() != -1 and conn.fileno() != -1:
                ...
                if len(r) != 0:
                    ...
                    msg_string = msg_bytes.decode('utf-8').removesuffix('\n').removeprefix('ACK ')
                    ...
                    try:
                        ...
                        if ack_num == 0:
                            ...
                            window_start_index = ack_num + int(window_start_index / window_size)*window_size
                        elif (window_end_index + 1) == ack_num:
                            window_start_index = ack_num + int(window_end_index / window_size)*window_size
                            window_end_index = ack_num + int(window_end_index / window_size)*window_size
                            timestamp = time.time()
                    except ValueError:
                        pass
