...
int main()
{
    ...
    int data = 0, offset=0, bpp = 0, wt, ht;
    long imgsize = 0, bmpdataoff = 0;
    int** image;
    int temp = 0;
    ...
    else
    {
        ...
        offset = 10;
        fseek(fp, offset, SEEK_SET);
        fread(&bmpdataoff, 4, 1, fp);
        fseek(fp, 18, SEEK_SET);
        fread(&wt, 4, 1, fp);
        fread(&ht, 4, 1, fp);
        fseek(fp, 2, SEEK_CUR);
        fread(&bpp, 2, 1, fp);
        fseek(fp, bmpdataoff, SEEK_SET);
        image = (int**)malloc(ht * sizeof(int*));
        for (i = 0; i < ht; i++)
        {
            image[i] = (int*)malloc(wt * sizeof(int));
        }
        int numbytes = (imgsize - bmpdataoff) / 3;
        for (i = 0; i < ht; i++)
        {
            for (j = 0; j < wt; j++)
            {
